...
class _MenuBagiHasilPageState extends State<MenuBagiHasilPage> {
  ...
  Future<void> _refreshData() async {
    final freshData = await _dbHelper.getMudharabahById(_currentAkadData['id']);
    if (freshData != null && mounted) {
      setState(() {
        _currentAkadData = freshData;
      });
    }
  }
  ...
  Future<void> _hapusAkad() async {
    bool confirm = await showDialog(
          ...
          builder: (context) => AlertDialog(
            ...
            content: const Text(
                "Yakin ingin menghapus akad ini? Data yang dihapus tidak dapat dikembalikan."),
            actions: [
              TextButton(
    ...
    if (confirm) {
      ...
      await db.delete('mudharabah_akad',
          where: 'id = ?', whereArgs: [_currentAkadData['id']]);
      if (mounted) {
        Navigator.pop(context); // Kembali ke list
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text("Akad berhasil dihapus")),
        );
      }
    }
  }
  ...
  Future<void> _updateStatus(String newStatus) async {
    await _dbHelper.updateAkadMudharabah(_currentAkadData['id'], {
      'nama_usaha': _currentAkadData['nama_usaha'],
      'deskripsi_usaha': _currentAkadData['deskripsi_usaha'],
      'nominal_modal': _currentAkadData['nominal_modal'],
      'nisbah_nasabah': _currentAkadData['nisbah_nasabah'],
      'nisbah_bmt': _currentAkadData['nisbah_bmt'],
      'status': newStatus
    });
  ...
  Widget build(BuildContext context) {
    ...
    double modal = (_currentAkadData['nominal_modal'] as num?)?.toDouble() ?? 0;
    double nNasabah =
        (_currentAkadData['nisbah_nasabah'] as num?)?.toDouble() ?? 0;
    double nBmt = (_currentAkadData['nisbah_bmt'] as num?)?.toDouble() ?? 0;
